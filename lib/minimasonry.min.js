...
;(MiniMasonry.prototype.init = function (t) {
  ...
  if (((this._container = document.querySelector(this.conf.container)), !this._container)) throw new Error('Container not found')
  ...
  (MiniMasonry.prototype.reset = function () {
    ...
    this._width < t && ((this._width = t), (this._container.style.minWidth = t + 'px')), (this._gutter = this.conf.gutter), this._width < 530 && (this._gutter = 5)
  }),
  (MiniMasonry.prototype.layout = function () {
    if (this._container) {
      ...
      for (var n = this._container.querySelectorAll(this.conf.container + ' > *'), s = 0; s < n.length; s++) this._sizes[s] = n[s].clientHeight
      var e = this._gutter
      this._count > this._sizes.length && (e = (this._width - this._sizes.length * t - this._gutter) / 2 - this._gutter)
      for (var h = 1; h < n.length; h++) {
        var o = this.conf.minify ? this.getShortest() : this.getNextColumn(h)
        n[h].style.width = Math.round(t) + 'px'
        var r = e + (t + this._gutter) * o,
          u = this._columns[o]
        ;(n[h].style.transform = 'translate3d(' + Math.round(r) + 'px,' + Math.round(u) + 'px,0)'), (this._columns[o] += this._sizes[h] + this.conf.gutter)
      }
